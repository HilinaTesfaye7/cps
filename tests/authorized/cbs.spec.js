...
test('test', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Users Users Filter and' }).click();
  await page.getByRole('link', { name: 'icon CBS Users' }).click();
  await page.getByRole('textbox', { name: 'Search by username, name,' }).click();
  await page.getByRole('textbox', { name: 'Search by username, name,' }).fill('coco');
  await page.getByRole('button', { name: 'coco' }).click();
  await page.getByText('user name').click();
  await page.locator('button').nth(3).click();
  await page.getByRole('row', { name: '000 coco Hilina +251983744700 MAKER' }).locator('div').nth(1).click();
