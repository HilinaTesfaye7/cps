...
test('test', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Minimum Transfer Amount' }).click();
  await page.getByText('Minimum Transfer AmountBack').click();
  await page.locator('header').getByRole('img').click();
  await page.locator('div').filter({ hasText: /^Maximum Transfer CapsSet maximum transfer limits for all transactions\.$/ }).nth(1).click();
  await page.getByRole('link', { name: 'icon Single & Daily Cap' }).click();
  await page.getByPlaceholder('search by service name....').click();
  await page.getByPlaceholder('search by service name....').fill('transfer to dashen bank');
